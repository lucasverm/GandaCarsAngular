...
export class BusChauffeurService {
  ...
  addBusChauffeur$(voornaam: string, achternaam: string, uurloon: string, email: String, geboorteDatum: Date): Observable<BusChauffeur> {
    return this.http.post<BusChauffeur>(`${environment.apiUrl}/BusChauffeur`, { voornaam, achternaam, uurloon, email, geboorteDatum }, { responseType: "json" }).pipe(
      catchError((error) => {
        return throwError(error);
      }),
      map(
  ...
  putBusChauffeur$(bc: BusChauffeur): Observable<BusChauffeur> {
    return this.http
      .put<BusChauffeur>(
        `${environment.apiUrl}/BusChauffeur/${bc.id}`,
        {
          id: bc.id,
          ...
          uurloon: bc.uurloon,
          email: bc.email,
          geboorteDatum: bc.geboorteDatum,
        },
        { responseType: "json" }
      )
      .pipe(
